...
// Bare-minimum health check — works even before everything else loads
...
// Load all the heavy stuff AFTER the server is already responding
async function loadFullApp() {
  try {
    ...
    // Seed (best-effort, never crashes)
    try {
    ...
    console.error('💥 Failed to load full app:', err.stack || err);
    // Server still listens on /api/health, /api/ping so we can debug
  }
}
