...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, username.trim(), email.toLowerCase().trim(), hash, location || '', avatar_color);

    const user = db.prepare('SELECT id, username, email, location, avatar_color FROM users WHERE id = ?').get(id);
